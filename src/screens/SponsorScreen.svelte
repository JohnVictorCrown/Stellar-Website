...
<div class="flex flex-col h-full overflow-y-auto no-scrollbar w-full px-2 sm:p-6 lg:p-10 xl:p-12 items-center">
  ...
  <div class="w-full mt-4 lg:mt-8">
    <div class="bg-[var(--color-surface)] border border-white/10 rounded-2xl p-4 sm:p-5 lg:p-8 xl:p-10 flex flex-col items-center">
      <h2 class="text-base sm:text-lg lg:text-2xl xl:text-3xl font-bold text-[var(--color-secondary)]">Become a Partner</h2>
      <p class="text-xs sm:text-sm lg:text-base xl:text-lg text-[var(--color-on-surface)] mt-1 lg:mt-3 text-center leading-relaxed">
      ...
      <button onclick={onContact} class="mt-4 lg:mt-6 px-6 py-2 lg:px-8 lg:py-3 bg-[var(--color-tertiary)] text-black font-bold uppercase tracking-wider text-[10px] lg:text-xs xl:text-sm rounded-full hover:opacity-90 active:scale-95 transition-all">
        Contact for a Deal
      </button>
    </div>
  </div>

  <h2 class="text-xs lg:text-sm xl:text-base font-semibold text-gray-400 mt-6 lg:mt-10 mb-3 lg:mb-5 uppercase tracking-widest text-center">Choose Payment Method</h2>
  ...
  <div class="grid grid-cols-2 gap-3 sm:gap-4 lg:gap-6 w-full pb-16 lg:pb-24">
    <button onclick={() => selectedMethod = 'bank'} class="h-24 sm:h-28 lg:h-32 xl:h-36 bg-[var(--color-surface)] border border-white/5 rounded-2xl lg:rounded-3xl flex flex-col justify-center items-center hover:bg-white/5">
      ...
      <span class="text-white font-semibold text-xs lg:text-sm xl:text-base tracking-wide">Bank Deposit</span>
    </button>
    <button onclick={() => selectedMethod = 'crypto'} class="h-24 sm:h-28 lg:h-32 xl:h-36 bg-[var(--color-surface)] border border-white/5 rounded-2xl lg:rounded-3xl flex flex-col justify-center items-center hover:bg-white/5">
      ...
      <span class="text-[var(--color-tertiary)] font-semibold text-xs lg:text-sm xl:text-base tracking-wide">Crypto</span>
    </button>
    <button onclick={() => selectedMethod = 'patreon'} class="h-24 sm:h-28 lg:h-32 xl:h-36 bg-[var(--color-surface)] border border-white/5 rounded-2xl lg:rounded-3xl flex flex-col justify-center items-center hover:bg-white/5">
      ...
      <span class="text-white font-semibold text-xs lg:text-sm xl:text-base tracking-wide">Patreon</span>
    </button>
    <button onclick={() => selectedMethod = 'paypal'} class="h-24 sm:h-28 lg:h-32 xl:h-36 bg-[var(--color-surface)] border border-white/5 rounded-2xl lg:rounded-3xl flex flex-col justify-center items-center hover:bg-white/5">
      ...
      <span class="text-[var(--color-secondary)] font-semibold text-xs lg:text-sm xl:text-base tracking-wide">PayPal</span>
    </button>
  </div>
</div>
...
<BottomSheet isOpen={selectedMethod !== null} onClose={() => selectedMethod = null}>
  {#if selectedMethod === 'bank'}
    <div class="flex flex-col items-center w-full">
      <h2 class="text-2xl font-bold text-white mb-2 text-center">Bank Transfer</h2>
      <p class="text-sm text-[var(--color-on-surface)] text-center mb-4">Please choose the account matching your currency.</p>
      <a href="https://www.notion.so/Stellarium-Literature-19fc1c04bbc1801f9243d1fa5d7d44ad?pvs=21" target="_blank" rel="noopener noreferrer" class="px-4 py-2 bg-[var(--color-surface)] border border-white/10 rounded-full text-xs text-[var(--color-tertiary)] font-bold uppercase tracking-wider mb-6 hover:bg-white/5 transition-colors">
      ...
      <div class="w-full space-y-4">
        {#each BANK_DETAILS as bank}
          <div class="bg-white/5 border border-white/10 rounded-xl p-4 w-full">
            <h3 class="text-[var(--color-primary)] font-bold mb-1">{bank.title}</h3>
  ...
  {:else if selectedMethod === 'crypto'}
    <div class="flex flex-col items-center w-full">
      <h2 class="text-2xl font-bold text-white mb-6 text-center">Cryptocurrency</h2>
      <div class="w-full bg-white/5 border border-white/10 rounded-xl p-4 mb-6">
        <h3 class="text-[var(--color-secondary)] font-bold mb-4">Monero (XMR)</h3>
      ...
      <a href="https://trocador.app/en/anonpay" target="_blank" rel="noopener noreferrer" class="w-full py-4 text-center bg-[var(--color-tertiary)] text-black rounded-full font-bold uppercase tracking-wider">
        Pay with Any Crypto (Trocador)
      </a>
    </div>
  {:else if selectedMethod === 'patreon'}
    <div class="flex flex-col items-center w-full">
      <h2 class="text-2xl font-bold text-[var(--color-primary)] mb-4 text-center">Patreon</h2>
      <p class="text-[var(--color-on-surface)] text-center mb-8">Join our exclusive community on Patreon.</p>
      <a href="https://www.patreon.com/join/StellariumFoundation" target="_blank" rel="noopener noreferrer" class="w-full max-w-xs py-4 text-center bg-white text-black rounded-full font-bold uppercase tracking-wider">
        Visit Patreon Page
      </a>
    </div>
  {:else if selectedMethod === 'paypal'}
    <div class="flex flex-col items-center w-full">
      <h2 class="text-2xl font-bold text-white mb-6 text-center">PayPal</h2>
      ...
      <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=stellar.foundation.us@gmail.com&currency_code=USD" target="_blank" rel="noopener noreferrer" class="w-full max-w-xs py-4 text-center bg-white text-black rounded-full font-bold uppercase tracking-wider">
        Send via PayPal
      </a>
    </div>
  {/if}
